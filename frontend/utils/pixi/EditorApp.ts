...
export class EditorApp extends App {
    ...
    private loadAssets = async () => {
        await Promise.all([
            PIXI.Assets.load(`${PUB.sprites}/tile-outline.png`),
            PIXI.Assets.load(`${PUB.sprites}/erase-tile.png`),
            PIXI.Assets.load(`${PUB.sprites}/collider-tile.png`),
            PIXI.Assets.load(`${PUB.sprites}/teleport-tile.png`),
            PIXI.Assets.load(`${PUB.sprites}/spawn-tile.png`),
            PIXI.Assets.load(`${PUB.sprites}/private-tile.png`),
        ])
    }
    ...
    private placeSpawnTileSprite = (x: number, y: number) => {
        ...
        this.spawnTile = new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/spawn-tile.png`))
    ...
    private placeColliderSprite = (x: number, y: number, tile?: PIXI.Sprite) => {
        const key = `${x}, ${y}` as TilePoint
        const sprite = tile || new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/collider-tile.png`))
        sprite.x = x * 32
        sprite.y = y * 32
    ...
    private placePrivateAreaSprite = (x: number, y: number, tile?: PIXI.Sprite) => {
        const key = `${x}, ${y}` as TilePoint
        const sprite = tile || new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/private-tile.png`))
        sprite.x = x * 32
        sprite.y = y * 32
        this.gizmoContainer.addChild(sprite)
        this.gizmoSprites[key] = sprite
    ...
    private placeTeleportSprite = (x: number, y: number, tile?: PIXI.Sprite) => {
        const key = `${x}, ${y}` as TilePoint
        const sprite = tile || new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/teleport-tile.png`))
        sprite.x = x * 32
        sprite.y = y * 32
        this.gizmoContainer.addChild(sprite)
        this.gizmoSprites[key] = sprite
    ...
    private drawGridLines = () => {
        this.gridLines = new PIXI.TilingSprite({
            texture: PIXI.Texture.from(`${PUB.sprites}/tile-outline.png`),
    ...
    private onRectangleEraseDragMove = (e: PIXI.FederatedPointerEvent) => {
        ...
        squares.forEach(square => {
            // place eraser tile
            const sprite = new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/erase-tile.png`))
    ...
    private getCurrentSpriteInfo = (): { data: SpriteSheetTile, layer: Layer | 'gizmo', tile: PIXI.Sprite, type: string } => {
        if (this.specialTileMode === 'Impassable') {
            const colliderTile = new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/collider-tile.png`))
            const layer = 'gizmo'
            return { data: {} as SpriteSheetTile, layer, tile: colliderTile, type: 'Impassable' }
        } else if (this.specialTileMode === 'Teleport') {
            const teleportTile = new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/teleport-tile.png`))
            const layer = 'gizmo'
            return { data: {} as SpriteSheetTile, layer, tile: teleportTile, type: 'Teleport' }
        } else if (this.specialTileMode === 'Spawn') {
            const spawnTile = new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/spawn-tile.png`))
            const layer = 'gizmo'
            return { data: {} as SpriteSheetTile, layer, tile: spawnTile, type: 'Spawn' }
        } else if (this.specialTileMode === 'Private Area') {
            const privateAreaTile = new PIXI.Sprite(PIXI.Texture.from(`${PUB.sprites}/private-tile.png`))
            const layer = 'gizmo'

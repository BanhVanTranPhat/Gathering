...
const Editor: React.FC<EditorProps> = ({ realmData }) => {
  ...
  const [selectedTile, setSelectedTile] = useState<TileWithPalette>({
    name: "",
    palette: "ground",
  });
  ...
  const [rooms, setRooms] = useState<string[]>(
    realmData.rooms.map((room) => room.name),
  );
  const [roomIndex, setRoomIndex] = useState<number>(0);
  const [selectedPalette, setSelectedPalette] = useState<SheetName>(
    palettes[0],
  );
    ...
    <div className="relative w-full h-screen flex flex-col">
      ...
      <div className="w-full grow flex flex-row">
        <LeftBar
          ...
          selectEraserLayer={selectEraserLayer}
        />
        <PixiEditor
